...
public class UptimeCommandTests
{
    ...
    private readonly Mock<IStreamMonitor> _streamMonitor;
    public UptimeCommandTests()
    {
        _streamMonitor = new Mock<IStreamMonitor>();
    }

    [Fact]
    public async Task NotStreaming_Returns_StreamerIsntLive()
    {
        _streamMonitor.SetupGet(x => x.IsStreaming).Returns(false);

        var target = new UptimeCommand(_streamMonitor.Object);
    ...
    {
        _streamMonitor.SetupGet(x => x.IsStreaming).Returns(true);
        var streamStarted = DateTime.Now.AddHours(-hoursStreamed).AddMinutes(-minutesStreamed).AddSeconds(-secondsStreamed);
        _streamMonitor.SetupGet(x => x.StreamStarted).Returns(streamStarted);
