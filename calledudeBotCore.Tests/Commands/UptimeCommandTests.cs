...
public class UptimeCommandTests
{
	...
	{
		_streamingState.SetupGet(x => x.IsStreaming).Returns(false);

		var target = new UptimeCommand(_streamingState.Object);
	...
	{
		_streamingState.SetupGet(x => x.IsStreaming).Returns(true);
		var streamStarted = DateTime.Now.AddHours(-hoursStreamed).AddMinutes(-minutesStreamed).AddSeconds(-secondsStreamed);
		_streamingState.SetupGet(x => x.StreamStarted).Returns(streamStarted);
